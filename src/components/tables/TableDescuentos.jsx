...
export default function TablesDescuentos() {
    ...
    const getTableData = async () => {
        try {
            const { data } = await clienteMongoAxios("/api/discount/getAll");
            setDatos(data);
        } catch (error) {
    ...
    const descargarDescuento = async (id) => {
        try {
            const response = await clienteMongoAxios.get(`/api/discount/${id}/invoice`, {
                responseType: 'blob',
            });

            if (response.status !== 200) {
                throw new Error(`Error al generar la descuento: ${response.statusText}`);
            }
            ...
            a.click();
            a.remove();
            toast.success('Descuento Descargada con Éxito', {
            ...
            toast.error('Error al Descargar Descuento', {
                position: toast.POSITION.TOP_CENTER,
                autoClose: 3000,
                hideProgressBar: false,
                closeOnClick: true,
            ...
            console.error('Error al descargar la descuento:', error);
        }
    };

    const enviarDescuentoPorWhatsApp = async (id) => {
        try {
            const response = await clienteMongoAxios.get(`/api/discount/${id}/invoice`, {
                responseType: 'blob',
            });
            ...
            const mensaje = `Hola, aquí está la descuento solicitada: ${url}`;
            const enlaceWhatsApp = crearEnlaceWhatsApp(numeroTelefono, mensaje);

            window.open(enlaceWhatsApp, '_blank');

            toast.success('Descuento enviada por WhatsApp');
        } catch (error) {
            console.error('Error al enviar la descuento por WhatsApp:', error);
            toast.error('Error al enviar la descuento por WhatsApp');
        }
    };
                                ...
                                <tr key={dato.id}>
                                    ...
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <div className="flex items-center space-x-3.5">
                                            ...
                                            {/* <button
                                                onClick={() => descargarDescuento(dato.id)}
